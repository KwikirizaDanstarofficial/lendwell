...
export function InterestRatesClient({ saccoId }: { saccoId: string }) {
  const { data: rateRows = [] } = useQuery(
    "SELECT * FROM interest_rates WHERE sacco_id = ? ORDER BY min_amount ASC",
    [saccoId]
  )
  ...
  const wrappedAction = async (_prev: InterestRateActionState, formData: FormData) => {
    const data = {
      minAmount: Number(formData.get("min_amount")),
      maxAmount: Number(formData.get("max_amount")),
      rate: Number(formData.get("rate")),
      rateType: (formData.get("rate_type") ?? "monthly") as "daily" | "monthly" | "annual",
    }
    return addInterestRateAction(data)
  }
    ...
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Define interest rate tiers based on loan amount ranges.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            The system auto-selects the applicable rate based on the loan amount.
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {rates.map((rate) => (
                  <TableRow key={rate.id}>
                    <TableCell>{formatUGX(rate.minAmount * 100)}</TableCell>
                    <TableCell>{formatUGX(rate.maxAmount * 100)}</TableCell>
